...
public class RedBoxMachine
{
   ...
   ArrayList<DVD> lib = new ArrayList<DVD>();
   /** the list of DVDs */
   ...
   {
      for (DVD DVD : lib){
         if (DVD.getTitle().equals(title) && DVD.getNumCopies() > 0){
            return lib.indexOf(DVD);
         }
      }
      return -1;
   ...
   public boolean rent(String title)
   {
      if (searchForMovie(title) >= 0){
         lib.get(searchForMovie(title)).decrementCopies();
         return true;
      }
      return false;
   ...
   {
      if (searchForMovie(title) >= 0){
         lib.get(searchForMovie(title)).incrementCopies();
      }
      else{
         lib.add(new DVD(title));
      }
      return lib.get(searchForMovie(title));
   }
